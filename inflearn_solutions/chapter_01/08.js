...
function solution(arr) {
    ...
    for (let i=0; i<arr.length; i++) { // 배열의 0번부터  총 9명의 난쟁이를 for문으로 돌고
        for (let j=i+1; j<arr.length; j++) { // 0번 옆에 있는 난쟁이부터 총 9명의 난쟁이들을 또 for문으로 돌면서
            // console.log(i, j, '=', arr[i], arr[j]);  // 이거 찍어보면 이제 명확하지 !
    ...
    return answer; // 얕은 복사이기 때문에 answer나 arr아무거나 써도 둘 다 원본 배열인 arr를 가리킨다. 
