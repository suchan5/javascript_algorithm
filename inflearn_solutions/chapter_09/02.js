...
function solution (n, arr) { // n : 정점의 개수, arr : 간선 정보를 배열로 넣었다
    ...
    for(let [a, b] of arr) { // for문 돌면서 인접행렬을 만들어보자
        console.log(a, b);
        graph[a][b]=1; // 이렇게하면 방향 그래프가 된다. 예) graph[1][2]=1; 1행 2열에 체크되었다는 소리
    }

    function dfsR(v) { // L대신 v사용. 그래프에서 정점에서 정점으로 이동하니까 
        if (v===n) { // 종착점은 dfsR(5)지 이 문제에서. 그러므로 n은 5
            answer++; // 종착점 5번 정점에 오면 쁠쁠해준다
        } else {
            for (let i=1; i<n+1; i++) { // n개의 가지 개수만큼 for문을 돌아준다. 1번부터입니다 정점 번호는. 
                if (graph[v][i] === 1 && check[i] === 0) { // v라는 정점에서 i라는 그래프로 갈 수 있느냐 확인해야 되잖아. 내가 지금 v에서 i로 갈 수 있느냐, 갈 수 있다면 1이겠지 . 그리고(&&) 체크배열의 i갈래가 체크가 되지 않아야(0) 갈 수 있다
