...
//         } else {
//             for(let i=1; i<=n; i++) { // n은 입력값 배열의 길이(arr.length)랑 같은 소리다. 왼쪽(1번 구슬) , 중간(2번 구슬), 오른쪽(3번 구슬) 세가닥이 있다. for문을 돌리는게 더 똑똑해보이지! ㅋㅋ
...
// 이렇게 구조를 바꿔봤다. 이게 나한테 이해하기 더 편하다
function solution(arr, m) { 
    ...
    function dfsR(L) { //L이 몇 중 for문을 도느냐를 결정
        if (L===m) { // m번 뽑아야 level의 트리의 마지막 종착점까지 간거다. 
            ...
            for(let i=0; i<arr.length; i++) { // n은 입력값 배열의 길이(arr.length)랑 같은 소리다. 왼쪽(1번 구슬) , 중간(2번 구슬), 오른쪽(3번 구슬) 세가닥이 있다. for문을 돌리는게 더 똑똑해보이지! ㅋㅋ
                tmp[L]=arr[i]; // tmp라는 임시 배열을 만들고 tmp[L]에 i(=왼쪽(1번 구슬), 중간(2번 구슬), 오른쪽(3번 구슬))을 순차적으로 넣는다. 
                dfsR(L+1);
            }
        }
    }
...
let arr = [1, 2, 3]
console.log(solution(arr, 2)); // 1~3까지 적힌 구슬 세 개가 있고, 총 두 번을 뽑는다
